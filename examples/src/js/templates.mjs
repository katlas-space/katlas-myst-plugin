...
const mystTemplate = {
    ...
    run(data, vfile, ctx) {
        console.log('[Templates] Run ctx:', Object.keys(ctx));
        console.log('[Templates] Run vfile path:', vfile.path);
        console.log('[Templates] Run data:', data);
...
function templateTransform(opts, utils) {
    console.log('[Templates] Plugin opts:', JSON.stringify(opts, null, 2));
    return async (mdast, vfile, session) => {
        console.log('[Templates] Transform running for:', vfile.path);
        if (session) {
            console.log('[Templates] Session available:', Object.keys(session));
            console.log('[Templates] Session config (keys):', Object.keys(session.config || {}));
            const siteMermaid = session.config?.site?.options?.mermaid;
            console.log('[Templates] Session site.options.mermaid:', JSON.stringify(siteMermaid, null, 2));
        } else {
            console.log('[Templates] Session NOT available in transform');
        }
        console.log('[Templates] vfile.data.frontmatter:', JSON.stringify(vfile.data?.frontmatter, null, 2));

        // Check for mermaid config in site options (if available) or passed opts
        // Sometimes site config is merged into opts or vfile.data?
        console.log('[Templates] Check site.options.mermaid in opts/vfile?');
        ...
        // Also check code blocks for mermaid opts as requested
        utils.selectAll('code', mdast).forEach((node) => {
            if (node.lang === 'mermaid') {
                console.log('[Templates] Found mermaid code block:', node);
            }
        });
